<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Helper\Cart;
use App\Models\Category;
use App\Models\Product;

class CartController extends Controller
{
    public function listproduct(Cart $cart)
    {
        $cartItems = $cart->getList();
        $product_cart = Product::paginate(5);
        $data_category = Category::paginate(3);

        return view('cart', compact('cart', 'product_cart', 'data_category', 'cartItems'));
    }

    public function add(Request $request, Cart $cart)
    {
        $product = Product::find($request->id);
        $quantity = $request->quantity ?? 1;

        if ($product) {
            $cart->add($product, $quantity);
        }

        return redirect()->route('cart.product')->with('message', 'Đã thêm sản phẩm vào giỏ hàng');
    }

    public function remove($productId, Cart $cart)
    {
        if ($cart->remove($productId)) {
            return redirect()->back()->with('message', 'Sản phẩm đã được xóa khỏi giỏ hàng');
        }

        return redirect()->back()->with('error', 'Sản phẩm không tồn tại trong giỏ hàng');
    }

    public function showCart(Cart $cart)
    {
        return view('cart', ['cart' => $cart->getList()]);
    }
}
